...
struct Vector3
{
    double xyz[3];
    int index;
    Vector3(double x, double y, double z, int idx)
    {
        xyz[0] = x;
        xyz[1] = y;
        xyz[2] = z;
...
int main(int argc, char** argv)
{
    ...
    sscanf(argv[3], "%lf", &ss1);
    sscanf(argv[4], "%lf", &ss2);
    
    boxsize = (round)(1.2 / cell);
    for (int i = 0; i < pnumber; i++)
    {
        fscanf(pointreader, "%lf %lf %lf", &po[i].xyz[0], &po[i].xyz[1], &po[i].xyz[2]);
        po[i].index = i;
        int ppp = floor(((po[i].xyz[0] + 0.6) / cell)) * boxsize * boxsize + floor(((po[i].xyz[1] + 0.6) / cell)) * boxsize + floor(((po[i].xyz[2] + 0.6) / cell));
        search.push(ppp);
    }
    kd.build(0, pnumber - 1);
    int count = 0;
    while (!search.empty())
    {
        count++;
        ...
        int x = tppp;
        Vector3 center;
        center.xyz[0] = x * cell + 0.5 * cell - 0.6;
        center.xyz[1] = y * cell + 0.5 * cell - 0.6;
        center.xyz[2] = z * cell + 0.5 * cell - 0.6;
        kd.query(center, 10);
        Vector3 pt[10];
        ...
        {
            dist[tpp] = tdist;
            if (tdist >= ss1 && tdist <=ss2)
            {
                fprintf(targetwriter, "%lf %lf %lf %d %d %d\n", center.xyz[0], center.xyz[1], center.xyz[2],pt[minidx].index,pt[8].index, pt[9].index);
            }
            else if (tdist > ss2)
                continue;
            for (int i = 0; i < 6; i++)
